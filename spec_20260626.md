...
Calling it non-interactively (from Script-Fu console or another plug-in)
python# From Filters > Development > Python-Fu > Python Console:
image    = Gimp.get_default_image()   # or Gimp.list_images()[0]
pdb      = Gimp.get_pdb()
proc     = pdb.lookup_procedure("python-fu-export-image")
cfg      = proc.create_config()
cfg.set_property("run-mode",    Gimp.RunMode.NONINTERACTIVE)
cfg.set_property("image",       image)
cfg.set_property("drawables",   image.get_active_drawable())
cfg.set_property("output-path", "/tmp/my-export.png")
proc.run(cfg)

Key notes
TopicDetailFormat selectionThe format is automatic based on the file extension (.png, .jpg, .webp, .tiff, etc.).FlatteningThe plug-in duplicates the image and flattens it before export, so your working layers are untouched. Remove flat_image.flatten() if you want to export a multi-layer format like TIFF.gimp-file-overwriteThis PDB procedure writes to a path without showing any dialog, which is what you want for scripted export.Paths on WindowsUse forward slashes: C:/Users/me/Desktop/out.png.GIMP restartAfter placing the file in the plug-ins folder, restart GIMP once so it registers the new procedure.